...
SMOKE_REDIS_CONFIG_KEY="anonimizador:config:smoke-ha:$(date +%s)"

run "REDIS_CONFIG_KEY=$SMOKE_REDIS_CONFIG_KEY sudo docker compose -f docker-compose.ha.yml up -d --build"
run "sleep 5"
run "REDIS_CONFIG_KEY=$SMOKE_REDIS_CONFIG_KEY sudo docker compose -f docker-compose.ha.yml ps"
...
run "REDIS_CONFIG_KEY=$SMOKE_REDIS_CONFIG_KEY sudo docker compose -f docker-compose.ha.yml stop web1 web2 web3 web4 web5"
run "sleep 3"
...
run "REDIS_CONFIG_KEY=$SMOKE_REDIS_CONFIG_KEY sudo docker compose -f docker-compose.ha.yml start web1 web2 web3 web4 web5"
run "sleep 15"
